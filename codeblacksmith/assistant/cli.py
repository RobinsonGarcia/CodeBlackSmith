...
def __check_if_model_exists(model_name):
...
def start_assistant(model_name, base_dir, temperature, num_ctx):
    try:
        ...
        sys.exit(1)
    llm = loader.get_model() # ChatOllama(model=model_name, temperature=temperature, num_ctx=num_ctx)
...
def start_server(model_name, base_dir):
    ...
    subprocess.run(["python", "-m", "codeblacksmith.assistant.web_assistant.app", "--model", model_name, "--base-dir", base_dir])
